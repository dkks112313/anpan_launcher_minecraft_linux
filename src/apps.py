...
class MainWindow(QWidget):
    ...
    def default_directory(self):
        os.makedirs(os.path.join(pathlib.Path.home(), '.local', 'share', 'launch'), exist_ok=True)
        settings['minecraft_directory'] = os.path.join(pathlib.Path.home(), '.local', 'share', 'launch')
    ...
    def open_directory(self):
        if os.path.isdir(os.path.join(self.path_box.text(), self.version_select.currentText())):
            subprocess.Popen("nautilus " + self.path_box.text() + '/' + self.version_select.currentText())
        else:
    ...
    def load_settings(self):
        ...
        if config['CONFIG']['directory'] == "":
            self.path_box.setText(os.path.join(pathlib.Path.home(), '.local', 'share', 'launch'))
        else:
    ...
    def launch_minecraft(self):
        ...
        if status.check_internet_connection():
            if self.path_box.text() != os.path.join(pathlib.Path.home(), '.local', 'share', 'launch'):
        ...
        if self.java_box.text() != "":
            options['executablePath'] = os.path.join(self.java_box.text(), 'java')
        ...
        if status.check_internet_connection():
            if settings['mods'] == 'Forge':
                if file_work.read_version_and_check(os.path.join(settings['minecraft_directory'], 'launch', f'Forge {settings['version']}')):
                    if os.path.isdir(os.path.join(settings['minecraft_directory'], 'launch', f'Forge {settings['version']}')):
                        self.show_message("Minecraft is starting")
            elif settings['mods'] == 'Fabric':
                if file_work.read_version_and_check(os.path.join(settings['minecraft_directory'], 'launch', f'Fabric {settings['version']}')):
                    if os.path.isdir(os.path.join(settings['minecraft_directory'], 'launch', f'Fabric {settings['version']}')):
                        self.show_message("Minecraft is starting")
            elif settings['mods'] == 'Qulit':
                if file_work.read_version_and_check(os.path.join(settings['minecraft_directory'], '.launch', f'Qulit {settings['version']}')):
                    if os.path.isdir(os.path.join(settings['minecraft_directory'], 'launch', f'Qulit {settings['version']}')):
                        self.show_message("Minecraft is starting")
            else:
                if file_work.read_version_and_check(os.path.join(settings['minecraft_directory'], '.launch', settings['version'])):
                    if os.path.isdir(os.path.join(settings['minecraft_directory'], 'launch', settings['version'])):
                        self.show_message("Minecraft is starting")
        else:
            if settings['mods'] == 'Forge':
                if file_work.read_version_and_check(os.path.join(settings['minecraft_directory'], 'launch', f'Forge {settings['version']}')):
        ...
        if self.exit_checkbox.isChecked() and file_work.read_version_and_check(
                os.path.join(settings['minecraft_directory'], 'launch', settings['version'])):
            self.close()
